...
getRequestsAndLimits () {
    ...
    data=$(oc get pods ${NAMESPACE} -o json | jq -r '.items[] | .metadata.namespace + "," + .metadata.name + "," + .spec.nodeName + "," + (.spec.containers[] | .name + "," + .resources.requests.cpu + "," + .resources.requests.memory + "," + .resources.limits.cpu + "," + .resources.limits.memory)')
    ...
    if [ "${HEADERS}" == true ]; then
        echo "Namespace,Pod,Node,Container,CPU request,CPU Usage,Memory request,Memory Usage,CPU limit,Memory limit" > "${OUT}"
    else
        echo -n "" > "${OUT}"
    fi
	...
	node=$(echo "${l}" | awk -F, '{print $3}')
        container=$(echo "${l}" | awk -F, '{print $4}')
        cpu_request=$(formatCpu "$(echo "${l}" | awk -F, '{print $5}')")
        mem_request=$(formatMemory "$(echo "${l}" | awk -F, '{print $6}')")
        cpu_limit=$(formatCpu "$(echo "${l}" | awk -F, '{print $7}')")
        mem_limit=$(formatMemory "$(echo "${l}" | awk -F, '{print $8}')")

        # Adding pod and container actual usage with pod top data
        line=$(oc adm top pod -n ${namespace} ${pod} --containers | grep " ${container} ")

        cpu_usage=$(formatCpu "$(echo "${line}" | awk '{print $3}')")
        memory_usage=$(formatMemory "$(echo "${line}" | awk '{print $4}')")

        final_line=${namespace},${pod},${node},${container},${cpu_request},${cpu_usage},${mem_request},${memory_usage},${cpu_limit},${mem_limit}
        if [ "${QUITE}" == true ]; then
            echo "${final_line}" >> "${OUT}"
        else
